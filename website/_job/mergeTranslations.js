...
try {
  ...
  console.error(
    " \x1b[31m✗\x1b[0m Error: Unable to read translations directory.",
    error,
  );
  process.exit(1);
}
...
languages.forEach((lang) => {
  ...
  try {
    ...
    console.error(
      ` \x1b[31m✗\x1b[0m Error: Unable to read language directory: ${langDir}`,
      error,
    );
    return;
  }
  ...
  if (jsonFiles.length === 0) {
    console.warn(
      ` \x1b[33m⚠\x1b[0m Warning: No JSON files found in ${langDir}`,
    );
    return;
  }
  ...
  jsonFiles.forEach((file) => {
    ...
    try {
      ...
      console.error(
        ` \x1b[31m✗\x1b[0m Error parsing JSON file: ${filePath}`,
        error,
      );
    }
  });
  ...
  try {
    ...
    console.log(
      ` \x1b[32m✓\x1b[0m Created merged translation file: ${outputFilePath}`,
    );
  } catch (error) {
    console.error(
      ` \x1b[31m✗\x1b[0m Error writing merged file: ${outputFilePath}`,
      error,
    );
  }
});
...
console.log(
  ` \x1b[32m✓\x1b[0m Merge process completed in ${(endTime - startTime) / 1000}s\n`,
);
